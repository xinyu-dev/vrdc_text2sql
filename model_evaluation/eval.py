# This file is copied from https://github.com/defog-ai/sql-eval/blob/main/eval/eval.py
...
def query_postgres_db(
    ...
    try:
        ...
        db_url = f"postgresql+psycopg2://{db_creds['user']}:{db_creds['password']}@{db_creds['host']}:{db_creds['port']}/{db_name}?options={db_creds['options']}"
...
def query_postgres_temp_db(
    ...
    try:
        ...
        admin_db_url = f"postgresql+psycopg2://{db_creds['user']}:{db_creds['password']}@{db_creds['host']}:{db_creds['port']}/postgres?options={db_creds['options']}"
...
def compare_query_results(
    ...
    timeout: float = 50.0,
    decimal_points: int = None,
) -> "tuple[tuple[bool, bool], pd.DataFrame, pd.DataFrame]":
    """
    ...
    for q in queries_gold:
        ...
        if type(results_gold) == list:
            if None in results_gold:
                return None, None, None
        
        if type(results_gen) == list:
            if None in results_gen:
                return results_gen, None, None
        
        if results_gold.empty and results_gen.empty:
            gold_columns = list(set(results_gold.columns)).sort()
            gen_columns = list(set(results_gen.columns)).sort()
            if gold_columns == gen_columns:
                return (True, True), results_gold, results_gen
            else:
                return (False, False), results_gold, results_gen
        elif results_gold.empty or results_gen.empty:
            return (False, False), results_gold, results_gen
        ...
        if compare_df(results_gold, results_gen, query_category, question, query_gold, query_gen):
            ...
            correct = True
    return (False, correct), results_gold, results_gen
