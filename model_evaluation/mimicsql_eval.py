...
def parse_args():
    ...
    args.add_argument("--current_time", type=str, default=datetime.strptime("2105-12-31 23:59:00", "%Y-%m-%d %H:%M:%S"))
...
def main(args):
    ...
    for idx, line in enumerate(pred):
        ...
        # query_real.append(postprocess_sql_query_from_markdown(line
        # ['output']))
        ...
        # query_real.append(postprocess_sql_query_from_markdown(real_sql).lower())
        query_real.append(postprocess_sql_query_from_markdown(real_sql))
        # Handle predict/pred
        pred_sql = line.get('predict') if 'predict' in line else line.get('pred')
        if pred_sql is None:
            query_pred.append('n/a')
        else:
            # query_pred.append(postprocess_sql_query_from_markdown(line
            # ['predict']))
            pred_sql_processed = postprocess_sql_query_from_markdown(pred_sql)
            if pred_sql_processed is not None:
                query_pred.append(pred_sql_processed)
            else:
                query_pred.append('n/a')
            # query_pred.append(postprocess_sql_query_from_markdown(pred_sql).lower())
            # query_pred.append(postprocess_sql_query_from_markdown(pred_sql.strip('`"\n')))
